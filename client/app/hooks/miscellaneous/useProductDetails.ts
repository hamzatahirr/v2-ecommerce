...
export const useProductDetail = () => {
  ...
  const onSubmit = async (data: ProductFormData) => {
    // Validate that variants don't have duplicate attribute combinations
    const variantAttributeKeys = data.variants.map((variant) => {
      const validAttributes = (variant.attributes || []).filter(
        (attr) => attr.valueId && attr.valueId.trim() !== ""
      );
    ...
    const duplicateKeys = variantAttributeKeys.filter(
      (key, index) => variantAttributeKeys.indexOf(key) !== index
    );

    if (duplicateKeys.length > 0) {
      showToast(
        "Duplicate attribute combinations detected. Please ensure each variant has unique attribute combinations.",
        "error"
      );
      return;
    }
    ...
    data.variants.forEach((variant, index) => {
      ...
      // Filter out attributes with empty valueId to prevent duplicate combinations
      const validAttributes = (variant.attributes || []).filter(
        (attr) => attr.valueId && attr.valueId.trim() !== ""
      );
      payload.append(
