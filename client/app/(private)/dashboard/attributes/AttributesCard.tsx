  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-300">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="space-y-1">
              {(attribute.categories || []).map((catRel, index) => (
                <div
                  key={`${catRel.id}-category-${index}`}
      ...
      <div className="p-4">
        ...
        {!isExpanded && (attribute?.values?.length ?? 0) > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {attribute.values?.slice(0, 3).map((value, index) => (
              <span
                key={`${value.id}-preview-${index}`}
                className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-full"
              >
                {value.value}
              </span>
            ))}
            {!isExpanded && (attribute?.values?.length ?? 0) > 3 && (
              <span className="px-2 py-1 bg-gray-100 text-gray-500 text-xs rounded-full">
                +{(attribute?.values?.length ?? 0) - 3} more
              </span>
            )}
          </div>
        )}
