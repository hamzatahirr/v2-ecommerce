...
const SupportPage = () => {
  ...
  const { data, isLoading } = useGetUserChatsQuery(undefined);
  ...
  const handleCreateChat = async () => {
    try {
      // This is for admin/support to create conversation with a user
      // For now, create a general support conversation
      const result = await createChat({ userId: 'support' }).unwrap();
      console.log("create chat result => ", result);
      const newChatId = result.data.conversation.id;
