...
import { logout, switchRole, ActiveRole } from "@/app/store/slices/AuthSlice";
...
const UserMenu = ({ menuOpen, closeMenu, user }) => {
  ...
  const roleOptions = [
    {
      role: "BUYER" as ActiveRole,
      label: "Shopping Mode",
      icon: <ShoppingBag size={18} className="text-blue-500" />,
      href: "/shop",
      available: true, // Everyone can access shopping
    },
    // Seller option - sellers use isSeller boolean
    {
          ...
          <div className="py-2 max-h-[calc(100vh-200px)] overflow-y-auto">
            ...
            {hasMultipleRoles && (
              <div className="mb-3 pb-3 border-b border-gray-200">
                ...
                {roleOptions
                  .filter((option) => option.available && option.role !== undefined)
                  .map((option) => (
                    <motion.div key={option.role} variants={itemVariants}>
                      <button
                        onClick={() => handleRoleSwitch(option.role as ActiveRole)}
                        className={`w-full flex items-center px-4 py-2.5 gap-3 hover:bg-gray-50/80 text-gray-700 text-sm transition-colors duration-150 relative group ${
                          activeRole === option.role ? "bg-indigo-50" : ""
                        }`}
                      >
