...
const Topbar = ({
  ...
  const { activeRole, isSeller, isAdmin, isAuthenticated } = useAuth();
  ...
  const fallbackConfig = {
    ...
    shippingText: "Free Shipping on Orders Rs 5000+",
  };
  ...
  // Determine background color based on active role
  const getRoleBackground = () => {
    if (!isAuthenticated) return "bg-indigo-950";
    if (isSeller) return "bg-green-950";
    if (isAdmin) return "bg-purple-950";
  ...
  const getRoleBadge = () => {
    ...
    if (isSeller) {
      return (
        <div className="flex items-center gap-1 px-2 py-1 bg-green-800/50 rounded-full text-xs">
          <Store size={12} />
    ...
    if (activeRole === "ADMIN" && isAdmin) {
      return (
        <div className="flex items-center gap-1 px-2 py-1 bg-purple-800/50 rounded-full text-xs">
          <Shield size={12} />
    ...
    return (
      <div className="flex items-center gap-1 px-2 py-1 bg-blue-800/50 rounded-full text-xs">
        <ShoppingBag size={12} />
  ...
  return (
    <div
      className={`${getRoleBackground()} text-white relative transition-all duration-300 ease-in-out ${
