...
export class AuthController {
  ...
  switchProfile = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const result = await this.authService.switchProfile(userId, profileType);

      sendResponse(res, 200, {
        data: result,
        message: `Switched to ${profileType} profile successfully`,
      });

      const start = Date.now();
      const end = Date.now();

      this.logsService.info("Profile switched", {
        userId,
        profileType,
        sessionId: req.session.id,
        timePeriod: end - start,
