  ...
  exportAnalytics = asyncHandler(async (req: Request, res: Response) => {
    ...
    switch (type) {
      case "overview":
        ...
        data = await this.analyticsService.getAnalyticsOverview(query, sellerId);
        filename = `analytics-overview-${new Date().toISOString()}.${format}`;
        break;
      case "products":
        const productSellerId = req.user?.isSeller ? req.user.id : undefined;
        data = await this.analyticsService.getProductPerformance(query, productSellerId);
