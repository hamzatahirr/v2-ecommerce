...
export class ProductService {
  ...
  async getAllProducts(queryString: Record<string, any>, sellerId?: string) {
  ...
  async updateProduct(
    ...
    // Authorization: Sellers can only update their own products, admins can update any
    if (!isAdmin && (!sellerId || existingProduct.sellerId !== sellerId)) {
      throw new AppError(
        403,
  ...
  async deleteProduct(productId: string, sellerId?: string, isAdmin: boolean = false) {
    const product = await this.productRepository.findProductById(productId);
    if (!product) {
      throw new AppError(404, "Product not found");
    }

    // Authorization: Sellers can only delete their own products, admins can delete any
    if (!isAdmin && (!sellerId || product.sellerId !== sellerId)) {
      throw new AppError(
        403,
