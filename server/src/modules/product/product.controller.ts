...
export class ProductController {
  ...
  getSellerProducts = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const {
        ...
        currentPage,
        resultsPerPage,
      } = await this.productService.getAllProducts(req.query, sellerId);
      sendResponse(res, 200, {
        data: {
          products,
          totalResults,
  ...
  createProduct = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      // Get sellerId from authenticated user (if they're a seller)
      const sellerId = req.user?.isSeller ? userId : undefined;
      if (!sellerId ) {
        throw new AppError(403, "Only sellers can create products");
      }
  ...
  updateProduct = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      // FIX: Handle BOTH formats (array or form-data key notation)
      let parsedVariants: any[] = [];

      // Case 1: Frontend sends JSON array: variants: [...]
      if (Array.isArray(req.body.variants)) {
        parsedVariants = req.body.variants;
      }

      // Case 2: Frontend sends form-data fields: variants[0][sku]
      else {
        for (const key in req.body) {
          if (key.startsWith("variants[")) {
            const match = key.match(/^variants\[(\d+)\]\[(\w+)\]$/);
            if (match) {
              const index = parseInt(match[1]);
              const field = match[2];
              if (!parsedVariants[index]) parsedVariants[index] = {};
              parsedVariants[index][field] = req.body[key];
            }
          }
        }
      ...
      const processedVariants = parsedVariants.length
        ? await Promise.all(
            parsedVariants.map(async (variant: any, index: number) => {
              ...
              variant.lowStockThreshold = Number(variant.lowStockThreshold);

              // Validate other fields
              if (!variant.sku || isNaN(variant.price) || isNaN(variant.stock)) {
                throw new AppError(
                  400,
  ...
  deleteProduct = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      await this.productService.deleteProduct(productId, sellerId, isAdmin);
